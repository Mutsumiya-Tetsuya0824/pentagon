...
<?php if(is_page(['uiux','mobile']) || is_singular('works')):
	$relatedposts = get_field('related_blog');
	if(!empty($relatedposts)):  ?>
	<div id="js-img" class="p-services__archive-list__wrap">
		<h2 class="p-services__archive-list__heading">スマホアプリの開発を外注する前に読んでおきたい記事</h2>
	...
	<?php
      ...
      <article class="archive-list__item">
        <a href="<?= esc_url(get_the_permalink($posts)); ?>">
          <figure class="img"><img src="<?= esc_url(get_the_post_thumbnail_url($posts)); ?>" alt=""></figure>
        </a>
        <div class="arichive-list__text">
          <h3 class="article-title"><?= esc_attr(get_the_title( $posts )); ?></h3>
          <div class="article-meta">
              <div class="category"><?= esc_attr(get_the_category( $p )[0]->name); ?></div>
            <div class="date">
              <time datetime="<?= esc_attr(get_the_date('Y-m-d',$posts)); ?>"><?= esc_attr(get_the_date('Y年m月d日',$posts)); ?></time>
            </div>
          </div>
        </div>
      </article>
...
<?php endif; 	?>
	<?php get_template_part('pentagon-parts/cta'); ?>
<?php endif; ?>
