...
?>
<!DOCTYPE html>
<html <?php echo isset($luxe['amp']) ? 'amp ' : $cls;
			language_attributes(); ?> itemscope itemtype="https://schema.org/WebPage">
	...
	?>
		...
		if (!isset($luxe['amp'])) {
			...
			?>
				<script>
					<?php
					echo	'(function(html){html.className=html.className.replace(/\bno-js\b/,"js")})(document.documentElement);',	thk_simple_css_minify(thk_fgc(TPATH . DSEP . 'js' . DSEP . 'lozad.min.js') . thk_fgc(TPATH . DSEP . 'js' . DSEP . 'thk-intersection-observer.min.js'));
					?>
				</script>
		<?php
			}
	 ...
	 ※ AMP の時はヘッダー内に置けないので body 直下配置
		...
		?>
			...
			<?php if(is_page_template( 'page_flutter.php' )){
				get_template_part('pentagon-parts/lp_header');
			}else{ ?>
				<?php
				if (is_singular('post')) {
					get_template_part('pentagon-parts/header_form');
				}
				?>
<?php if ( is_page_template( 'page_top_old.php' ) ) : ?>
				<!-- 旧header -->
			<header style="display: none;" id="header" itemscope itemtype="https://schema.org/WPHeader" <?php if (isset($luxe['add_role_attribute'])) echo ' role="banner"'; ?>>
					...
					<div id="head-in">
						...
						?>
							...
							?>
									<script>
										! function(e, o) {
											var t = o.getElementById("head-parallax");
											if (null !== t) {
												var l = o.getElementById("head-parallax"),
													n = o.getElementsByClassName("head-cover");
												t.style.overflow = "hidden", e.addEventListener("scroll", function() {
													var e, t = o.documentElement.scrollTop || o.body.scrollTop;
													0 < 2 * l.offsetHeight - t && (e = t / <?php echo $head_parallax ?> + "px", l.style.backgroundPosition = "0 " + e, n[0].style.transform = "translate(0," + e + ")")
												})
											}
										}(window, document);;
									</script>
...
<?php else: ?>
			...
			<header style="" class="headerWrap" itemscope itemtype="https://schema.org/WPHeader" <?php if (isset($luxe['add_role_attribute'])) echo ' role="banner"'; ?>>
				<div class="headerInnenr">
					<div class="headerLogo"><a href="<?php echo home_url(); ?>"><img
					src="<?php echo get_theme_file_uri('/assets/img/materials/logo/logo_h.svg'); ?>" alt="アプリ開発パートナーPentagon"></a></div>
					<div class="headerNavArea">
						<div id="head-band">
							<div class="bandNenu">
							<div class="headerBox">
							<?php $wp_nav_menu = wp_nav_menu(array('theme_location' => 'head-band', 'container' => false, 'fallback_cb' => false, 'items_wrap' => '<ul>%3$s' )); ?>
								</ul>	
									<div class="btnArea">
										<div class="splogo">
										<img
										src="<?php echo get_theme_file_uri('/assets/img/materials/logo/logo_h.svg'); ?>" alt="アプリ開発パートナーPentagon">
											</div>
											<div class="btnList">
												<div><a href="<?php echo home_url(); ?>/contact/" class="btn_type01">お問い合わせ</a></div>
												<div><a href="<?php echo home_url(); ?>/downloads/" class="btn_type01 bg-w">資料請求<span class="sp">する</span></a></div>
										</div>
									</div>
								</div>
							</div>
	...
	<?php endif; ?>
			...
			if ($luxe['bootstrap_header'] === 'out') {
				if ( !is_page_template('page_flutter.php') ) {
